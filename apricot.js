...
const ContentSchema = zod_1.z.object({
    start_timestamp: zod_1.z.string().nullable(),
    stop_timestamp: zod_1.z.string().nullable(),
    type: zod_1.z.string(),
...
const ConversationSchema = zod_1.z.object({
    ...
    chat_messages: zod_1.z.array(ChatMessageSchema),
});
const ChatHistorySchema = zod_1.z.array(ConversationSchema);
...
function processConversation(conversation, number, options) {
    var _a, _b;
    ...
    const inputRate = (_a = options.inputCost) !== null && _a !== void 0 ? _a : 3.0; // dollars per million input tokens
    const outputRate = (_b = options.outputCost) !== null && _b !== void 0 ? _b : 15.0; // dollars per million output tokens
    const totalCostDollars = (accInput / 1e6) * inputRate + (accOutput / 1e6) * outputRate;
    // In list view, print a header line with the conversation number, title, and cost.
    // Format: <number>  "<conversation name>" $<cost>
    console.log(`${number}  "${conversation.name}" $${totalCostDollars.toFixed(2)}`);
    ...
    if (options.tokens || options.verbose) {
        ...
        conversation.chat_messages.forEach((msg) => {
            ...
            if (msg.sender.toLowerCase() === "human") {
                ...
                console.log(`  human: msg=${msgTokens} input=${inputCost} output=0 acc-input=${localAccInput} acc-output=${localAccOutput}`);
            }
            else {
                const outputCost = msgTokens;
                localAccOutput += outputCost;
                console.log(`  assistant: msg=${msgTokens} input=0 output=${outputCost} acc-input=${localAccInput} acc-output=${localAccOutput}`);
            }
            runningHistory2 = newHistory;
        });
        // Print a detailed cost summary for this conversation.
        console.log(`  costs: input=$${inputRate}/MTok output=$${outputRate}/MTok input-costs=$${((localAccInput / 1e6) *
            inputRate).toFixed(2)} output-costs=$${((localAccOutput / 1e6) *
            outputRate).toFixed(2)} total-cost=$${((localAccInput / 1e6) * inputRate +
            (localAccOutput / 1e6) * outputRate).toFixed(2)}`);
        console.log();
    }
    ...
    try {
        ...
        if (options.conversationNumber) {
            ...
            if (idx >= chatHistory.length) {
                console.error(`Error: Conversation ${options.conversationNumber} does not exist`);
                process.exit(1);
            }
            processConversation(chatHistory[idx], options.conversationNumber, options);
            return;
        }
        ...
        if (options.tokens) {
            console.log();
            const inputRate = (_a = options.inputCost) !== null && _a !== void 0 ? _a : 3.0;
            const outputRate = (_b = options.outputCost) !== null && _b !== void 0 ? _b : 15.0;
